...
const GlassCard: React.FC<GlassCardProps> = ({ children, className = '', title, action }) => {
  return (
    <div className={`relative bg-white/70 backdrop-blur-2xl border border-white/60 shadow-glass rounded-2xl md:rounded-3xl overflow-hidden transition-all duration-300 transform-gpu ${className}`}>
      {title && (
