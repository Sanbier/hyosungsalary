...
interface DetailSectionProps {
  ...
  total?: string;
  totalLabel?: string;
  accentColor: string; // hex or tailwind class prefix logic
...
const DetailSection: React.FC<DetailSectionProps> = ({ 
  ...
  // Mapping accent colors to Tailwind classes
  ...
  return (
    <div className="bg-white/40 border border-white/50 rounded-2xl mb-3 overflow-hidden transition-all hover:bg-white/60">
      <div 
        onClick={() => setIsOpen(!isOpen)} 
        className="p-4 flex items-center justify-between cursor-pointer group"
      >
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-full ${theme.bg} ${theme.text} flex items-center justify-center shadow-sm`}>
            {icon}
          </div>
        ...
        <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${isOpen ? 'bg-gray-100 rotate-180' : 'bg-transparent text-gray-400 group-hover:bg-white'}`}>
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-4 pb-4 pt-0">
          <div className={`p-4 rounded-xl ${theme.bg} bg-opacity-30 border border-white/40 space-y-2.5 text-sm`}>
             {children}
          </div>
        </div>
      </div>
...
const Divider = () => <div className="h-px bg-gray-200/50 my-2"></div>;
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        <Divider />
        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Theo hệ số</h4>
        ...
        <Divider />
        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Phụ cấp Ca Đêm</h4>
      ...
      >
        <div className="text-xs text-gray-400 italic mb-2 text-center">Bảo hiểm tính trên: {formatVND(result.kt.luongTinhBH)}</div>
